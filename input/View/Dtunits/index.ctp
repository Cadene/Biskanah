<div class="dtunits index">
	<h2><?php echo __('Dtunits'); ?></h2>
	<table cellpadding="0" cellspacing="0">
	<tr>
			<th><?php echo $this->Paginator->sort('id'); ?></th>
			<th><?php echo $this->Paginator->sort('unit_id'); ?></th>
			<th><?php echo $this->Paginator->sort('building_id'); ?></th>
			<th><?php echo $this->Paginator->sort('begin'); ?></th>
			<th><?php echo $this->Paginator->sort('finish'); ?></th>
			<th><?php echo $this->Paginator->sort('num'); ?></th>
			<th><?php echo $this->Paginator->sort('num_ready'); ?></th>
			<th class="actions"><?php echo __('Actions'); ?></th>
	</tr>
	<?php foreach ($dtunits as $dtunit): ?>
	<tr>
		<td><?php echo h($dtunit['Dtunit']['id']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($dtunit['Unit']['id'], array('controller' => 'units', 'action' => 'view', $dtunit['Unit']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($dtunit['Building']['id'], array('controller' => 'buildings', 'action' => 'view', $dtunit['Building']['id'])); ?>
		</td>
		<td><?php echo h($dtunit['Dtunit']['begin']); ?>&nbsp;</td>
		<td><?php echo h($dtunit['Dtunit']['finish']); ?>&nbsp;</td>
		<td><?php echo h($dtunit['Dtunit']['num']); ?>&nbsp;</td>
		<td><?php echo h($dtunit['Dtunit']['num_ready']); ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $dtunit['Dtunit']['id'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $dtunit['Dtunit']['id'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $dtunit['Dtunit']['id']), null, __('Are you sure you want to delete # %s?', $dtunit['Dtunit']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>
	<div class="paging">
	<?php
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ''));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
</div>
<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Dtunit'), array('action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('List Units'), array('controller' => 'units', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Unit'), array('controller' => 'units', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('List Buildings'), array('controller' => 'buildings', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Building'), array('controller' => 'buildings', 'action' => 'add')); ?> </li>
	</ul>
</div>
